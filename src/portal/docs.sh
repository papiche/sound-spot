#!/bin/bash
# src/portal/docs.sh — Visionneuse de documentation offline
...
<style>
  :root {
    --black:#0a0a0f; --panel:#1a1a24; --accent:#7fff6e; --text:#e8e8f0; --muted:#7a7a99;
  ...
  body { background: var(--black); color: var(--text); font-family: var(--font-main); padding: 20px; line-height: 1.6; -webkit-font-smoothing: antialiased; }
  ...
  pre { background: #000; padding: 15px; overflow-x: auto; border-radius: 4px; border: 1px solid #2e2e42; margin: 1.5em 0; }
  code { font-family: var(--font-mono); color: #4ecdc4; font-size: 0.9em; background: rgba(0,0,0,0.3); padding: 2px 4px; border-radius: 3px; }
  ...
  blockquote { border-left: 4px solid var(--accent); margin: 1.5em 0; padding: 0.5em 15px; color: var(--muted); background: rgba(255,255,255,0.02); }
  .back-btn { display:inline-block; margin-bottom:20px; color:var(--muted); text-decoration:none; font-weight:bold; border:1px solid var(--muted); padding:5px 15px; border-radius:2px; font-family:var(--font-mono); font-size:0.8em; text-transform:uppercase; letter-spacing:1px; }
...
<body>
<div class="card">
  <a href="/" class="back-btn">← Retour au Portail</a>
HTMLEOF

FILE_PATH="${INSTALL_DIR}/${DOC}.md"
...
<p style="text-align:center; color:var(--muted); font-size:10px; font-family:var(--font-mono); margin-top:20px; text-transform:uppercase;">
  SoundSpot PicoPort // Documentation Hors-Ligne
</p>
</body>
</html>
HTMLEOF
