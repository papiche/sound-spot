...
if [ "$SOUNDSPOT_MODE" != "2" ]; then
    ...
    if [ -d "$PINOUT_DIR/output/en" ]; then
        ...
        # Réécrire les liens absolus pour le sous-chemin /pinout/
...
for d, _, files in os.walk(root):
    for f in files:
        ...
        if fixed != orig:
            ...
            n += 1
print(f'{n} fichiers HTML corrigés (liens absolus → /pinout/)')
PYEOF
...
# ════════════════════════════════════════════════════════════════
# Extrait dans src/dev/install_claude_tools.sh — peut aussi être lancé
# indépendamment de deploy_on_pi.sh : bash src/dev/install_claude_tools.sh
if [ "$SOUNDSPOT_MODE" != "2" ] && [ "${PICOPORT_ENABLED:-false}" = "true" ]; then
    bash "$SRC_DIR/dev/install_claude_tools.sh"
fi
